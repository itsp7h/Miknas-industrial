...
/**
 * What a document number will read given a code, without asking the server.
 *
 ...
 */
export function previewFor(code, savedNext, document = 'LPO') {
    const sequence = String(savedNext ?? '').split('-').pop() || '0001';
    const letters = (code ?? '').trim().toUpperCase();

    return letters
        ? `${letters}-${document}-${shortYear()}-${sequence}`
        : `${document}-${shortYear()}-${sequence}`;
}

/** Each company's document code, edited together and saved in one go. */
export default function useDocumentNumbering() {
    ...
    useEffect(() => {
        apiGet('/settings/document-numbering')
            ...
            .catch((err) => setError(err?.status === 403
                ...
                : 'Failed to load the document numbering.'))
    ...
    async function save() {
        ...
        setError('');
        try {
            const response = await apiPut('/settings/document-numbering', {
