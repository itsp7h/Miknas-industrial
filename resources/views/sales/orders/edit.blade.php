...
<div class="card card-body max-w-2xl">
    <form action="{{ route('sales.orders.update', $order) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="date" name="order_date" value="{{ old('order_date', $order->order_date ? \Carbon\Carbon::parse($order->order_date)->format('Y-m-d') : '') }}" required class="form-input">
            </div>

            <div>
                <label class="form-label">Delivery Date</label>
                <input type="date" name="delivery_date" value="{{ old('delivery_date', $order->delivery_date ? \Carbon\Carbon::parse($order->delivery_date)->format('Y-m-d') : '') }}" class="form-input">
            </div>
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('sales.orders.show', $order) }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
