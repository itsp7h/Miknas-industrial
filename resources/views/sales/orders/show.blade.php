...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">
            ...
            / {{ $salesOrder->order_number ?? 'SO-' . str_pad($salesOrder->id, 5, '0', STR_PAD_LEFT) }}
        </p>
    </div>
    <div class="flex gap-2">
        ...
        <a href="{{ route('sales.delivery-notes.create', ['sales_order_id' => $salesOrder->id]) }}" class="btn-primary">Create Delivery Note</a>
        @endif
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
    ...
    <div class="lg:col-span-2 card card-body">
        ...
        <dl class="grid grid-cols-2 gap-4 text-sm">
            <div>
                ...
                <dd class="font-mono font-semibold text-gray-800">{{ $salesOrder->order_number ?? 'SO-' . str_pad($salesOrder->id, 5, '0', STR_PAD_LEFT) }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Status</dt>
                <dd>
                    @php
                        $badgeClass = match($salesOrder->status ?? 'draft') {
                            'draft'     => 'badge-gray',
                    ...
                    @endphp
                    <span class="{{ $badgeClass }}">{{ ucfirst($salesOrder->status ?? 'draft') }}</span>
                </dd>
            </div>
            <div>
                <dt class="text-gray-500">Order Date</dt>
                <dd class="font-medium text-gray-800">{{ $salesOrder->order_date ? \Carbon\Carbon::parse($salesOrder->order_date)->format('d M Y') : '-' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Delivery Date</dt>
                <dd class="font-medium text-gray-800">{{ $salesOrder->delivery_date ? \Carbon\Carbon::parse($salesOrder->delivery_date)->format('d M Y') : '-' }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="card card-body">
        ...
        <dl class="space-y-2 text-sm">
            <div><dt class="text-gray-500">Name</dt><dd class="font-semibold text-gray-800">{{ $salesOrder->customer->name }}</dd></div>
            <div><dt class="text-gray-500">Contact</dt><dd class="text-gray-700">{{ $salesOrder->customer->contact_person }}</dd></div>
            <div><dt class="text-gray-500">Email</dt><dd class="text-gray-700">{{ $salesOrder->customer->email }}</dd></div>
            <div><dt class="text-gray-500">Phone</dt><dd class="text-gray-700">{{ $salesOrder->customer->phone }}</dd></div>
        </dl>
        @endif
    </div>
</div>
        ...
        <table class="table-base">
            ...
            <tfoot class="bg-gray-50">
                <tr>
                    ...
                    <td class="px-4 py-3 text-right font-bold text-gray-900">{{ number_format($salesOrder->total_amount, 2) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
