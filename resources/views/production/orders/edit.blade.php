    ...
    <form action="{{ route('production.orders.update', $productionOrder) }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                ...
                <select name="product_id" required class="form-select">
                    ...
                    @foreach($items as $item)
                        <option value="{{ $item->id }}" {{ old('product_id', $productionOrder->product_id) == $item->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="quantity_to_produce" value="{{ old('quantity_to_produce', $productionOrder->quantity_to_produce) }}" min="0.01" step="0.01" required class="form-input">
            </div>

            <div>
                <label class="form-label">Production Date <span class="text-red-500">*</span></label>
                <input type="date" name="production_date" value="{{ old('production_date', $productionOrder->production_date ? \Carbon\Carbon::parse($productionOrder->production_date)->format('Y-m-d') : '') }}" required class="form-input">
            </div>

            <div class="sm:col-span-2">
                <label class="form-label">Notes</label>
                <textarea name="notes" rows="3" class="form-textarea">{{ old('notes', $productionOrder->notes) }}</textarea>
            </div>
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('production.orders.show', $productionOrder) }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
